...
export class OrdersService {
  ...
  getAllOrder(status?: number): Observable<Orders[]> {
  ...
  getAllOrderByUserAndStatus(id: number, status: number): Observable<Orders[]> {
    return this.http.get<Orders[]>(API_URL + `/orders/users/${id}?status=` + status);
  }

  getAllOrderByUserAndStatusNotLike(id: number, status: number): Observable<Orders[]> {
    return this.http.get<Orders[]>(API_URL + `/orders/users/${id}/status-not-like?status=${status}`);
  }
  ...
  getAllProductByOrder(id: number): Observable<ProductImage[]> {
    return this.http.get<ProductImage[]>(API_URL + `/orders/${id}/products`);
  }
}
